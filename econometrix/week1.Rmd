...
Скалярное произведение двух векторов - это произведение длин этих векторов и 
...
\right.
$$
Соответственно мы можем проиллюстрировать метод наименьших квадратов для 
случая множества объясняющих переменных.

Заодно на этом рисунке мы увидим, где находится RSS, TSS и ESS.

Проиллюстрируем оценку модели: $y_i=\beta_1+\beta_2x_i+\beta_3z_i+\varepsilon_i$ с 
...
ggplot(sml, aes(x, y, group = name, color = name)) +
