...
fun1 <- function(path) {
# path <- "/Users/macmini/Desktop/OES_output/СЧЕТ-ФАКТУРА №  от 30 апреля 2016 г._5817.xlsx"
 df <- read_excel(path, sheet = 1, cell_cols(1:21), col_names = F) # для ДВ исправит
 bingo_row <- which(df$X__1 == "Всего к оплате")
 df_out <- data_frame("Счет-фактура" = c(df$X__1[2]),
                      ...
                      "Всего к оплате1" = c(df$X__13[bingo_row]), # для дв ИСПРавит для СЗ
                      "Всего к оплате2" = c(df$X__19[bingo_row]), # для дв ИСПРавит для СЗ
...
fun2 <- function(path) {
# path <- "/Users/macmini/Desktop/OES_output/Акт приема-передачи электрической энергии № К-17-06-50 от 29 февраля 2016 г. исправление №1 от 31 августа 2016 г._290.xlsx"
 df <- read_excel(path, sheet = 1, cell_cols(1:19), col_names = F)
 bingo_row <- which(df$X__2 == "ИТОГО")
 df_out <- data_frame("Акт приема-передачи" = c(df$X__1[8]),
                      ...
                      "Всего к оплате" = c(df$X__18[bingo_row]), # Южный ДВ
                      "ID" = str_extract(str_extract(path, "_[:digit:]+\\.xlsx$"), "[:digit:]+")
                        )
 return(df_out)
}
...
```{r}
fun3 <- function(path) {
  # path <- "~/Desktop/OES_output/КОРРЕКТИРОВОЧНЫЙ СЧЕТ-ФАКТУРА № Э-1893-02-77 от 30 ноября 2016 г. ИСПРАВЛЕНИЕ КОРРЕКТИРОВОЧНОГО СЧЕТА-ФАКТУРЫ № ---  от  ---_86.xlsx"
 df <- read_excel(path, sheet = 1, cell_cols(1:23), col_names = F)
 bingo_up <- which(df$X__1 == "Всего увеличение (сумма строк В)")
 bingo_down <- which(df$X__1 == "Всего уменьшение (сумма строк Г)")
 df_out <- data_frame("№ Корр счет фактуры" = c(df$X__1[3]),
                      ...
                      "Всего увеличение" = c(df$X__21[bingo_up]),
                      "Всего уменьшение" = c(df$X__21[bingo_down]),
                      "ID" = str_extract(str_extract(path, "_[:digit:]+\\.xlsx$"), "[:digit:]+")
                        )
 return(df_out)
}
...
```{r}
fun5 <- function(path) {
 # path <- "~/Desktop/OES_output/Корректировочный акт приема-передачи электрической энергии № Э-3831-02-50 от 30 ноября 2016 г. к акту приема-передачи электрической энергии № Э-3145-02-50 от 30 сентября 2016 г._55.xlsx"
 df <- read_excel(path, sheet = 1, cell_cols(1:25), col_names = F)
 bingo_up <- which(df$X__3 == "Всего увеличение")
 bingo_down <- which(df$X__3 == "Всего уменьшение")
 df_out <- data_frame("№ Корр акт" = c(df$X__1[7]),
                      ...
                      "Всего увеличение" = c(df$X__25[bingo_up]), # ДВ
                      "Всего уменьшение" = c(df$X__25[bingo_down]), # ДВ
                      "ID" = str_extract(str_extract(path, "_[:digit:]+\\.xlsx$"), "[:digit:]+")
                        )
 return(df_out)
}
