...
# R - язык программрования для научных вычислений и анализа данных с упором на
# визуализацию и воспроизводимость

# R - свободное кросс-платформенное програмное обеспечение с открытым
# исходным кодом

# R - интерперетируемый язык с интерфейсом командной строки

# R - ?шамултипарадигменный язык, сочетающий в себе
# - функциональное
# - процедурное
...
#### Урок 1.2.5
help("sin")
help('sin') # в R одинарные и двойные кавычкм равнозначны
help(`sin`) # даже можно бэктик использовать
help.search("sin")

help(sin) # можно вообще без кавычек

# но для некоторых зарезервированных слов кавычки обязательны
# help(for) - выдаст ошибку

?sin # так еще проще и быстрее
?'?' # справка по справке
...
# Не любое имя переменной считается валдидным!

# Глобальное окружение (Global Environment):
# Каждая прееменная живет в своем окружении
# Окружение переменной - это пространство, где ее можно найти
# Все переменные созданные в консоли, попадают в глобальное окружение

ls() # List Objects 
# - с пустыми скобками это список переменны в глобальном окружении  

## Встроенные функции

?rnorm # нормальное распределение
rnorm(15) # будет распечатано 15 чисел
rnorm(15, mean = 5, sd = 3) # mean - среднее, sd - стандартное отклонение
# заполнение аргументов по правилу ключ = значение

## Написани функций

return_two <- function() {
  y <- 2 # создаем переменную y в локальном окружении ф-ии
  return(y)
}
return_two()
...
## Окружение функции
addten <- function(x) {
  x <- x + 10 # мы переприсволили х в локальном окружении, и оно не попало в
                                                                # глобальное
}
...
#### Урок 1.2.11
# двоичный логарифм от квадрата синуса арктангенса единицы 
log2(sin(atan(1))^2)
...
# на векторах основаны более сложные структуры (матрица, список, 
                                              # вактор, дата фрейм)
...
# т.е. скаляр - это ветор длины 1
# Индексация начинается с единицы

# как бы это было в других языках:
x <- vector(length = 2)
x[1] <- 5
x[2] <- 8
x

x <- c(5, 8); x # c - combine; данная функция может быть вложенной и 
...
rep(1:3, times = 4)   # rep - replicate; times - сколько повторять САМ вектор
...
# Все трети и все седьмые части, отсортированные
u <- seq(0, 1, 1/3)
v <- seq(0, 1, 1/7)
c(u, v)
help.search('sort')
...
# Векторизация
# многие функции в языке имеют встроенную поддержку векторизации 
# vectorized
sqrt(1:4)
# vectorized - округление вниз до ближайшего целого
...
all.equal(make.names(v, unique = TRUE), v)
which(make.names(v, unique = TRUE) != v)

# рушим правила)))
assign("Сьешь еще этих мягких французских булок, да выпей чаю!", 1)
# а это от Антонова
`Съешь ещё` <- 1

rm(`Сьешь еще этих мягких французских булок, да выпей чаю!`, `Съешь ещё`)
...
# condition - это выражение, результатом которого будет логический вектор 
# длины 1 (T или F; NA - приведет к ошибке)

# Переносить else на новую строку нельзя! Т.к. команды в R выполняются построчно
...
# ifelse проверка вектора условий
...
# ключевые слова next и break
for (i in letters) {
  if (i == "c") next # пропуск буквы "с"
  if (i == "f") break # на этом условии цикл заканчивается
  print(i)
}

# внутри цикла необходимо вызывать print для вывода переменных
# в консоли x и print(x) равнозначны, а в цикле нет
for (i in 1:5) i # ничего не напечатается
...
}) # подсчет идет 62с на рабочем компе (29 на домашнем)
...
# Библиотека (в контексте экосистемы R) - это место на диске, где хранятся
# доступные пакеты

.libPaths() # список библиотек, где лежат пакеты
# CRAN - центральный репозиторий
# The Comprehensive R Archive Network (всеобъемлющий)
...
# library(pkgname) или require(pkgname)
...
# - Если длина большего вектора не делится нацело на длину меньшего, выдаётся 
# предупреждение

1:5 + 0:1
# выдаст предупреждение, но посчитает
...
x[] # весь вектор целиком

# Положительные индексы - выводит номер элемента
x[1]
x[3:7]
x[c(8, 7, 5, 6:9, x[1])] # рекурсия! :)
...
# Атрибут может быть с произвольным названием
attr(x, "autor") <- "Caesar"
attributes(x)
attributes(x) <- NULL
...
paste(letters[c(20, 8, 1, 14, 11, 25, 15, 21)], collapse = "")
paste(letters[c(13, 25, 16, 12, 5, 1, 19, 21, 18, 5)], collapse = "")

let_eng <- function(x) {
  paste(letters[x], collapse = "")
}

x1 <- c(7, 15, 15, 4, 20, 1, 19, 11)
let_eng(x1)
x2 <- c(13, 5, 18, 3, 9)
let_eng(x2)
x3 <- c(20,15,15,8,1,18,4,20,15,6,9,14,4,13,1,19,19,9,22,5)
let_eng(x3)
x4 <- c(19, 15, 18, 18, 25, 1, 2, 15, 21, 20, 20, 8, 1, 20)
let_eng(x4)
